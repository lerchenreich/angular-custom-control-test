import { Component, Input, Output, HostBinding, ElementRef, Optional, Self, EventEmitter, Directive  } from '@angular/core';
...
@Component({
  selector: 'code-input2',
  templateUrl: './lookup-control.component.html',
  styleUrls: ['./lookup-control.component.css'],
  providers: [{provide: MatFormFieldControl, useExisting: CodeControl2Component}],
  //encapsulation: ViewEncapsulation.None
})
export class CodeControl2Component extends MatFormFieldControl<string> implements ControlValueAccessor{
  static nextId = 0
  parts         : FormGroup = new FormGroup({'inputControl': new FormControl('')});
  stateChanges  : Subject<void> = new Subject<void>()
  focused       : boolean = false
